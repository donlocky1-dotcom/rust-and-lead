...
**Klassen-Chip (Politur):** Links an der Lebensleiste jedes Gegners sitzt ein kleiner
Klassen-Marker — **eckig + stahlblau = `MECHANICAL`** (galvanisch stark), **rund + fleischrot =
`BIOLOGICAL`** (kinetisch stark). Form *und* Farbe kodieren die Klasse (bewusst keine
mehrdeutigen Emojis), sodass die richtige Waffenwahl schon vor dem ersten Schuss ablesbar ist.
Ein einmaliger Toast beim ersten Gegner-Aggro erklärt die Kodierung. Zusammen mit den reaktiven
Stark/Schwach-Hinweisen und den Status-Icons bildet das die vollständige Kampf-Lesbarkeit.
...
> (Godot 4.3.stable, `godot --headless --path godot` → **340/340 Checks, Exit 0**), zusätzlich in
